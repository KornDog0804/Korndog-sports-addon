...
// jmp2.uk is a URL-shortener used by community iptv-org lists to wrap
// Samsung TV+ embed links. Those links contain literal placeholder tokens
// (like {PSID}, {TARGETOPT}) that only get filled in by Samsung's actual
// app - never by us. ffprobe sometimes tolerates the malformed redirect
// enough to "pass" verification, but real browsers/hls.js correctly reject
// it. Filter these out entirely rather than relying on verification to
// catch them.
function hasBrokenRedirectPattern(channel = {}) {
  const url = channel.streamUrl || '';
  return url.includes('jmp2.uk');
}

function removeBlockedChannels(channels) {
  return channels.filter(
    channel => !isBlockedChannel(channel) && !hasBrokenRedirectPattern(channel)
  );
}
